...
class Screen:
  def __init__(self, config, font_cache, image_cache):
    ...
    for cancelbutton in (config['cancelbuttons'] if config.has_key('cancelbuttons') else []):
      image = self.imageCache.getImage(cancelbutton['image'])
      cntButton = CancelButton(image, cancelbutton['position'], cancelbutton['event'])
      if cancelbutton.has_key('zindex'):
        cntButton.zindex = cancelbutton['zindex']
      self.controls.append(cntButton)
  ...
  def onevent(self, event):
    for control in self.controls:
      ...
      if isinstance(control, CancelButton):
        event = control.onevent(event)
...
class CancelButton:
  ...
  def onevent(self, event):
    ...
    event.button == 0 and self.state == Button.BTN_STATE_PUSHED) or\
    (event.type == pygame.JOYBUTTONUP and event.button == 0 and self.state == Button.BTN_STATE_PUSHED):
      if (event.type == pygame.MOUSEBUTTONUP and self.image.get_rect(center=self.center).collidepoint(event.pos)) or\
      event.type == pygame.JOYBUTTONUP:
        pygame.event.post(pygame.event.Event(Button.EVENT_BUTTONCLICK,
                  {'name':self.event}))
